...
module.exports = function(pid/*,procpath*/,cb){ // or task: ":pid/task/:tid"
  ...
  var o = {
    ...
    stat:function(cb){
      ...
      // });
      let buff = ''
      const stream = fs.createReadStream(procPath+pid+'/stat', {highWaterMark: (8 * 1024), encoding: 'ascii'});
      ...
      stream.on('end', () => {
        let values = buff.toString().trim().split(" ");
        cb(false,assoc(module.exports.fields['/proc/:pid/stat'],values))
        stream.destroy()
      });
    },
    ...
    statm:function(cb){
      ...
      // })
      let buff = ''
      const stream = fs.createReadStream(procPath+pid+'/statm', {highWaterMark: (8 * 1024), encoding: 'ascii'});
      stream.on('data', chunk => {
        buff += chunk
      });
      stream.on('error', err => {
      ...
      stream.on('end', () => {
        let values = buff.toString().trim().split(/\s+/g)
        cb(false,assoc(module.exports.fields['/proc/:pid/stat'],values))
        stream.destroy()
      });
    },
    ...
    status:function(cb){
      ...
      // });
      let buff = ''
      const stream = fs.createReadStream(procPath+pid+'/status', {highWaterMark: (8 * 1024), encoding: 'ascii'});
      stream.on('data', chunk => {
        buff += chunk
      });
      stream.on('error', err => {
    ...
    argv:function(cb){
      ...
      // });
      let buff = ''
      const stream = fs.createReadStream(procPath+pid+'/cmdline', {highWaterMark: (8 * 1024), encoding: 'ascii'});
      stream.on('data', chunk => {
        buff += chunk
      });
      stream.on('error', err => {
